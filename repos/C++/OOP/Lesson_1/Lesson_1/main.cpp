...
int generateR()
{
	vector<int> znamenatelVec;
    vector<int> chislitelVec;
	int start = 2;
	int chislitel;
	int znamenatel;
	int znToAdd = 1;
	int chislitelToAdd = 1;
	znamenatelVec.push_back(znToAdd);
	chislitelVec.push_back(chislitelToAdd);
	...
	auto znamenatelIterator = begin(znamenatelVec);
    for (auto i = begin(chislitelVec); i != end(chislitelVec); ++i)
    {
        if (*znamenatelIterator != 1)
            cout << *i << " / " << *znamenatelIterator << endl;
        else
            cout << *i << endl;
        ++znamenatelIterator;
    }
...
//vector<double_int> divec;
//vector<int> ivec;
//double_int dvoenInt;
//dvoenInt[0] = dvoenInt[1] = 0;
//cout << dvoenInt[0] << dvoenInt[1] << endl;
//int a = 1;
//ivec.push_back(a);
//char ch = 'a';
   //int i = 5;
  ...
  //bool someValue = 1;
  //sizeof(someValue ? ++a, ++i : --a, --i);
  //cout << sizeof(long long);
  long long N = 1;
    for (long long j = 2; j < 65; ++j)
    {
        cout << N << " * " << j << " = " << N*j << endl;
        N *= j;
    }
